...
@jit
def _levenshtein_distance(sent1, sent2):
    distances = numpy.zeros((len(sent1) + 1,len(sent2) +1))
        ...
        for t2 in range(1, len(sent2) + 1):
            if (sent1[t1-1] == sent2[t2-1]):
                ...
                if (a <= b and a <= c):
                    ...
                    distances[t1][t2] = c + 1
    return distances[len(sent1)][len(sent2)]


@jit
def _compare(sentences, comp_sent, date, certain, certain_dates, uncertain, uncertain_dates, uncertain_sents):
    for i, sent in enumerate(sentences):
        sent = sentences[i]
        d = _levenshtein_distance(sent, comp_sent)
        ...
        if certainty >= 0.8 and certain[i] < certainty:
            ...
            uncertain_dates[i] = -1
            uncertain_sents[i] = ""
        elif certain[i] == -1 and uncertain[i] < certainty:
            uncertain[i] = certainty
            uncertain_dates[i] = date
            uncertain_sents[i] = comp_sent

@jit
def _process_text(sentences, text, year, certain, certain_dates, uncertain, uncertain_dates, uncertain_sents):
    for line in text:
        if len(line) < 7:
            continue
        _compare(sentences, line, year, certain, certain_dates, uncertain, uncertain_dates, uncertain_sents)
    ...
    for year in os.listdir(in_path):
        ...
        for f in os.listdir(year_path):
            ...
            with open(file_path, "r") as reader:
                text = sent_tokenize(reader.read().strip())
                _process_text(sentences, text, year, certain, certain_dates, uncertain, uncertain_dates, uncertain_sents)
    data = {"sents": sentences, "certain": certain, "certain_years": certain_dates, "uncertain": uncertain,"uncertain_years": uncertain_dates,"comp_sents": uncertain_sents,  "left": sentences}
...
def write_hansard_dates():
    ...
    for index, row in hansard.iterrows():
        ...
        try:
            ...
            _process_text(sentences, deb_sents, row["date"], certain, certain_dates, uncertain, uncertain_dates, uncertain_sents)
        except Exception as e:
            print(e)
            continue
    data = {"sents": sent_copy, "certain": certain, "certain_years": certain_dates, "uncertain": uncertain,"uncertain_years": uncertain_dates,"comp_sents": uncertain_sents,  "left": sentences}
